...
8. "List" or enumeration queries MUST always include a numeric metric column — never return a label-only result set. Examples:
   ...
   - "best/top ONE [thing] per [group]" (best category per week, top product per day, leading customer per region, etc.) → concatenate group and winner into one label: SELECT (group_expr::TEXT || ' — ' || winner_name) AS label, SUM(oi.quantity * oi.unit_price) AS revenue ... QUALIFY RANK() OVER (PARTITION BY group_expr ORDER BY revenue DESC) = 1 ORDER BY label (bar) — always 2 columns so ChartRenderer renders correctly; NEVER return 3 columns (group, winner, metric) for this pattern
